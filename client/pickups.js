...
export function updatePickups(time) {
  ...
  for (const id in window._armorMeshes) { if (!seenArmor.has(id)) { const m = window._armorMeshes[id]; scene.remove(m); m.traverse(c => { if (c.geometry) c.geometry.dispose(); if (c.material) c.material.dispose(); }); delete window._armorMeshes[id]; } }

  // Weapon pickups — mini viewmodels
  ...
  for (const id in window._wpMeshes) { if (!seenWp.has(id)) { const g = window._wpMeshes[id]; scene.remove(g); g.traverse(c => { if (c.geometry) c.geometry.dispose(); if (c.material) { if (c.material.map) c.material.map.dispose(); c.material.dispose(); } }); delete window._wpMeshes[id]; } }

  // Food — procedural 3D models per type
