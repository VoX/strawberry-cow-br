...
function gameTick() {
  ...
  // Night hostile bot spawning — during night phase (720-1080s in the
  // 1200s cycle), periodically spawn aggressive bots at map edges.
  // Max 5 night bots alive at once. They use existing bot AI.
  const cycleT = gameState.getGameTime() % 1200;
  const isNight = cycleT >= 720 && cycleT < 1080;
  if (isNight) {
    // Count existing night bots
    let nightBotCount = 0;
    for (const [, p] of gameState.getPlayers()) {
      if (p.isBot && p._nightBot) nightBotCount++;
    }
