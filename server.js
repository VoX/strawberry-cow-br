...
function generateMap() {
  ...
  // Shelters (2-3) — protect from cowstrike
  const shelterCount = 2 + Math.floor(Math.random() * 2);
  for (let i = 0; i < shelterCount; i++) {
    SHELTERS.push({ x: rand(250, MAP_W - 250), y: rand(250, MAP_H - 250), r: rand(50, 70) });
  }
}
...
function spawnBots() {
  ...
  for (let i = 0; i < botsNeeded; i++) {
    ...
    const bot = {
      ...
      xp: 0, level: 0, xpToNext: 30, kills: 0,
...
function getLobbyPlayers() {
  const arr = [];
  for (const [, p] of players) {
    if (p.inLobby || p.alive) arr.push({ id: p.id, name: p.name, color: p.color });
  }
  return arr;
}
  ...
  for (const [, p] of players) {
    if (p.inLobby) {
      ...
      Object.assign(p, {
        ...
        foodEaten: 0, xp: 0, level: 0, xpToNext: 30, kills: 0,
        dashCooldown: 0, attackCooldown: 0, stunTimer: 0, lastAttacker: null,
        perks: { speedMult: 1, radiusMult: 1, drainMult: 1, magnetRange: 0, regen: 0, maxHunger: 100, sizeMult: 1, damage: 1 },
        weaponPerks: { velocity: 1, cooldown: 1, hungerDiscount: 0, extraProj: 0, damageMult: 1, piercing: false, burstMod: false },
        weapon: 'normal', weaponLevel: 0, weaponTimer: 0,
  ...
  for (const [, p] of players) {
    if (p.alive || (!p.inLobby && gameState === 'playing')) {
      arr.push({
        ...
        kills: p.kills || 0, stunTimer: p.stunTimer || 0, weapon: p.weapon || 'normal', weaponLevel: p.weaponLevel || 0,
    ...
    for (let i = foods.length - 1; i >= 0; i--) {
      ...
      if (Math.hypot(p.x - f.x, p.y - f.y) < collectRadius) {
        ...
        if (p.xp >= p.xpToNext) {
          ...
          p.level++;
          p.xpToNext = Math.floor(30 + p.level * 15 + p.level * p.level * 3);
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'perk' && player && player.alive && player.perks) {
      ...
      if (id === 'speed') perks.speedMult += 0.3;
        ...
        [3000, 4000, 5000].forEach((delay, wave) => {
          setTimeout(() => {
            if (gameState !== 'playing') return;
